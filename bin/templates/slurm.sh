...
launch_job(){{
	...
	sbatch \
		...
		<<- EOM
            ...
            # Optional: check GPUs from inside the container
			echo "Container GPU info:"
			apptainer exec --nv docker://{training_image} nvidia-smi
			...
			apptainer \
				...
				--bind "/scratch/\${{USER}}/models":/model-remote \
                ...
                --env LMUData="/data-remote" \
                --env HF_HOME="/model-remote/hugging_face" \
