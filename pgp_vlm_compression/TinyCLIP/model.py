...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, mlp_ratio: float = 4.0,
                 act_layer: Callable = nn.GELU, cut_layer: int = -1):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        self.cut_layer = layers - 1 if cut_layer==-1 else cut_layer
...
class VisualTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(
            width, layers, heads, mlp_ratio, act_layer=act_layer, cut_layer=cut_layer)
        self.head_dim = width // heads
    ...
    def forward(self, x: torch.Tensor,
        ...
        x_split = x_split.permute(1, 0, 2)  # LND -> NLD
...
class ImageEncoder(nn.Module):
    ...
    def forward(self, image, normalized=False,
        ...
        image_features, image_features_split = self.visual(image, **mask)
        ...
        return image_features, image_features_split
